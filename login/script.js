...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const playSound = (type) => {
        try {
            ...
            console.error("Web Audio API login engine failure:", e);
        }
    };
    ...
    // --- Forgot Password Toast Interaction ---
    const forgotPasswordLink = document.getElementById('forgot-password');
    if (forgotPasswordLink) {
        forgotPasswordLink.addEventListener('click', (e) => {
            e.preventDefault();
            playSound('click');
            showToast("Password recovery link sent to your email ✉️", "success", 3000);
        });
    }
    ...
    loginForm.addEventListener('submit', (e) => {
        ...
        const termsCheckboxLogin = document.getElementById('terms-checkbox');
        ...
        if (termsCheckboxLogin && !termsCheckboxLogin.checked) {
        ...
        if (!matchingUser) {
            // Shake both email and password boxes for stylish feedback
            emailBox.classList.add('shake');
            passwordBox.classList.add('shake');
            ...
            emailBox.addEventListener('animationend', clearShake, { once: true });
            passwordBox.addEventListener('animationend', clearShake, { once: true });
        ...
        // Login Success state execution
        safeSetItem("loggedIn", "true");
        playSound('success');
        showToast("Access granted. Loading command center... 🧘", "success", 2000);
